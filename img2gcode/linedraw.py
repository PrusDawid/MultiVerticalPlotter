...
# local imports
sys.path.insert(0, './line_lib') # (Import from lib folder)
from filters import *
...
def sketch(path):
    ...
    f.write(makesvg(lines, resolution/2, resolution*h//w/2))
    f.close()
...
def makesvg(lines, height, width):
    print("generating svg file...")
    out = '<svg xmlns="http://www.w3.org/2000/svg" version="1.1" viewBox="0 0 '+str(height)+' '+str(width) +' " >' #height="'+str(height)+'" width="'+str(width)+'">'
    for l in lines:
